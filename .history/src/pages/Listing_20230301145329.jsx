...
export default function Listing() {
  ...
  return (
    <div className="rounded-lg  mx-auto xl:w-9/12 lg:w-12/12 md:w-12/12 sm:w-12/12 mt-5">
      ...
      <div class="mx-auto ">
        <div className="flex items-center mb-3" onClick={() => useNavigate(-1)}>
          <IoChevronBackOutline />
          &nbsp;&nbsp; <p className="text-sm tracking-wide">Back to listings</p>
        </div>
        <section class="h-1/2">
      ...
      <div className="bg-slate-700 w-52 mt-3 p-3  text-lg font-bold rounded text-center hover:bg-slate-600">
        {auth.currentUser?.uid !== listing.userRef && (
          <Link
            to={`/contact/${listing.type}?${listing.userRef}?listingName=${listing.name}`}
          >
            Contact Landlord
          </Link>
        )}
